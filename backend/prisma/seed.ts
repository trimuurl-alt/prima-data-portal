import { PrismaClient, UserRole, UserStatus, DatasetCategory, DatasetStatus } from '@prisma/client';
...
async function main() {
  ...
  // ── Sample data manager ──
  const dmEmail = 'datamanager@primaresearch.com';
  let dm = await prisma.user.findUnique({ where: { email: dmEmail } });
  if (!dm) {
    dm = await prisma.user.create({
      data: {
        email: dmEmail,
        passwordHash: await bcrypt.hash('DataMgr@2026!', 12),
        fullName: 'Data Manager',
        role: UserRole.DATA_MANAGER,
        status: UserStatus.ACTIVE,
      },
    });
    console.log(`✓ Data manager created: ${dmEmail} / DataMgr@2026!`);
  }

  // ── Sample datasets (no files yet — admin uploads via UI) ──
  const samples = [
    { name: '2026 National School Dataset', slug: '2026-national-school-data', category: DatasetCategory.EDUCATION,
    ...
    { name: '2026 Shopping Mall Database', slug: '2026-shopping-mall-database', category: DatasetCategory.RETAIL,
    ...
    { name: 'Bash Store Locations', slug: 'bash-store-locations', category: DatasetCategory.RETAIL,
  ...
  for (const s of samples) {
    ...
    if (!exists) {
      ...
      // Grant access to sample client
      await prisma.datasetAccess.create({
        data: { userId: client.id, datasetId: created.id, grantedById: admin.id },
      });
    } else {
  ...
  console.log(`  Data manager: ${dmEmail} / DataMgr@2026!`);
