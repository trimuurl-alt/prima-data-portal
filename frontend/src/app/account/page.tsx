...
function ChangePasswordForm() {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (next !== confirm) return alert('Passwords do not match'); change.mutate(); }} className="space-y-4 max-w-md">
      ...
      <button type="submit" disabled={change.isPending} className="bg-brand-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-600 disabled:opacity-50">
...
function MfaSection({ mfaEnabled, onChange }: { mfaEnabled: boolean; onChange: () => void }) {
  ...
  if (setupData) {
    return (
      <div className="space-y-4 max-w-md">
        ...
        {confirmMfa.isError && <div className="bg-red-50 text-red-700 text-sm rounded-lg px-3 py-2">{getErrorMessage(confirmMfa.error)}</div>}
        <div className="flex gap-2">
          <button onClick={() => confirmMfa.mutate()} disabled={confirmMfa.isPending || code.length !== 6}
            className="bg-brand-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-600 disabled:opacity-50">
            {confirmMfa.isPending ? 'Verifying…' : 'Verify and enable'}
          </button>
  ...
  return (
    <div className="space-y-4 max-w-md">
      ...
      <button onClick={() => setup.mutate()} disabled={setup.isPending} className="bg-brand-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-600 disabled:opacity-50">
