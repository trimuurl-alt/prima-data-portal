      ...
      <div className="bg-white border border-stone-200 rounded-xl">
        <div className="px-4 py-3 border-b border-stone-200">
          <input
            ...
            className="w-full px-3 py-1.5 border border-stone-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-600"
          />
        </div>
  ...
  return (
    <Modal title="Invite user via email" onClose={onClose}>
      {link ? (
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <SelectField label="Role" value={form.role} onChange={(v) => setForm({ ...form, role: v })}
            options={[{ v: 'CLIENT', l: 'Client' }, { v: 'DATA_MANAGER', l: 'Data manager' }, { v: 'ADMIN', l: 'Admin' }]} />
      ...
      <div className="flex justify-end gap-2 px-6 py-4 bg-stone-50 border-t border-stone-200 rounded-b-xl">
        {link ? (
          <button onClick={onClose} className="px-4 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium">Done</button>
        ) : (
          <>
            <button onClick={onClose} className="px-4 py-2 border border-stone-300 rounded-lg text-sm">Cancel</button>
            <button onClick={() => invite.mutate()} disabled={invite.isPending} className="px-4 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium disabled:opacity-50">
