            ...
            <tbody>
              {datasets?.items?.map((d: any) => (
                <tr key={d.id} className="border-t border-stone-100 hover:bg-stone-50/60">
                  ...
                  <td className="px-2 py-3 text-stone-500 capitalize truncate">{d.category.toLowerCase()}</td>
                  <td className="px-2 py-3 text-stone-500 truncate uppercase">{d.fileType ?? '—'}</td>
                  <td className="px-2 py-3 text-right text-stone-500">{d._count?.access ?? 0}</td>
...
function QuickAction({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center gap-2.5 px-3 py-2 border border-stone-200 rounded-lg text-xs text-stone-900 hover:border-brand-400 hover:bg-brand-50/40 transition"
    >
      <span className="w-7 h-7 rounded-md bg-brand-50 text-brand-800 flex items-center justify-center group-hover:bg-brand-100 transition">
        {icon}
      </span>
...
function activityStyle(action: string) {
  if (action.startsWith('DATASET_'))
    return { bg: 'bg-brand-50', text: 'text-brand-800', icon: <Upload className="w-3 h-3" /> };
